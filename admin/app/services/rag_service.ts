...
export class RagService {
  ...
  public static CONTENT_COLLECTION_NAME = 'haven_knowledge_base'
    ...
    try {
      ...
      */
      ...
      if (resolvedSource.startsWith(uploadsAbsPath + sep)) {
        ...
        logger.warn(`[RAG] File was removed from knowledge base but doesn't live in Haven's uploads directory, so it can't be safely removed. Skipping deletion of physical file...`)
      }
  ...
  public async discoverHavenDocs(force?: boolean): Promise<{ success: boolean; message: string }> {
    try {
      ...
      if (alreadyEmbeddedRaw && !force) {
        logger.info('[RAG] Haven docs have already been discovered and queued. Skipping.')
        return { success: true, message: 'Haven docs have already been discovered and queued. Skipping.' }
      }
      ...
      logger.info(`[RAG] Discovered ${filesToEmbed.length} Haven doc files to embed`)
      ...
      return { success: true, message: `Haven docs discovery completed. Dispatched ${filesToEmbed.length} embedding jobs.` }
    } catch (error) {
      logger.error('Error discovering Haven docs:', error)
      return { success: false, message: 'Error discovering Haven docs.' }
    }
  }
  ...
  public async scanAndSyncStorage(): Promise<{
    ...
    try {
      ...
      // Force resync of Haven docs
      await this.discoverHavenDocs(true).catch((error) => {
        logger.error('[RAG] Error during Haven docs discovery in sync process:', error)
      })
